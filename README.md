# 📄 سياسة الخصوصية - Privacy Policy

## تطبيق تمّ — خدمات التكييف والطاقة الشمسية
## Tamm — HVAC & Solar Energy Services App

---

### 🔗 عرض السياسة الكاملة
**View Full Privacy Policy**

👉 [اضغط هنا لقراءة سياسة الخصوصية كاملة - Click here to read the full privacy policy](PRIVACY_POLICY.md)

---

### 📱 عن التطبيق - About the App

**تمّ** هو تطبيق متكامل لخدمات التكييف والطاقة الشمسية، يتيح للعملاء تصفح المنتجات وطلب الخدمات ومتابعة الطلبات بسهولة.

**Tamm** is a comprehensive HVAC and solar energy services app that allows customers to browse products, request services, and track orders easily.

---

### 🔐 الالتزام بالخصوصية - Privacy Commitment

نحن ملتزمون بحماية خصوصيتك وبياناتك الشخصية:

We are committed to protecting your privacy and personal data:

- ✅ **لا إعلانات** - No Advertisements
- ✅ **لا بيع للبيانات** - No Data Selling
- ✅ **تشفير كامل** - Full Encryption
- ✅ **شفافية تامة** - Complete Transparency

---

### 📋 ملخص السياسة - Policy Summary

#### البيانات المجمعة - Data Collected:
- الاسم ورقم الهاتف - Name and phone number
- البريد الإلكتروني - Email address
- عنوان التوصيل - Delivery address
- سجل الطلبات والمشتريات - Orders and purchase history

#### استخدام البيانات - Data Usage:
- إدارة الطلبات والتوصيل - Order management and delivery
- إرسال إشعارات حالة الطلب - Send order status notifications
- تحسين الخدمة - Improve service

#### الحماية - Protection:
- تخزين آمن على Supabase - Secure storage on Supabase
- تشفير SSL/TLS - SSL/TLS encryption
- عدم مشاركة مع أطراف ثالثة - No third-party sharing

---

### 📞 التواصل - Contact

**المطور - Developer**: صالح باقمري - Saleh Bagomri

**البريد الإلكتروني - Email**: s.bagomri@gmail.com

**الموقع - Website**: www.bagomri.com

---

### 📅 آخر تحديث - Last Updated

**مايو 2026 - May 2026**

**إصدار التطبيق - App Version**: 1.2.0

---

### 📖 اللغات المتاحة - Available Languages

- 🇾🇪 **العربية** - Arabic (النسخة الكاملة)
- 🇬🇧 **English** - English (Full Version)

---

### 🔗 روابط مهمة - Important Links

- **سياسة الخصوصية الكاملة** - [اقرأ هنا - Read here](PRIVACY_POLICY.md)
- **Google Play Store** - [قريباً - Coming soon]
- **موقع المطور** - [www.bagomri.com](https://www.bagomri.com)

---

### ⚖️ الامتثال - Compliance

هذه السياسة تتوافق مع:

This policy complies with:

- ✅ Google Play Store Policies
- ✅ GDPR Principles
- ✅ Yemen Data Protection Regulations

---

### 📝 الترخيص - License

هذا المستودع مخصص فقط لاستضافة سياسة الخصوصية لتطبيق تمّ.

This repository is dedicated solely to hosting the privacy policy for Tamm App.

---

💙 **تمّ — خدمات التكييف والطاقة الشمسية**

💙 **Tamm — HVAC & Solar Energy Services**

---

**© 2026 Saleh Bagomri - جميع الحقوق محفوظة - All Rights Reserved**
